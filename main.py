...
if __name__ == '__main__':
    ...
    if model or encoder or decoder is None:
        if not loaded:
            # mnist digits
    ...
    if args.noise:
        if loaded:
            ...
            # 95th %ile max training reconstruction on training with noise
            threshold = np.sort(noisy_reconstruction)[int(len(noisy_reconstruction)*.95)]
